...
```
unity/Assets/AngryGuy/Core/       the whole game: AI, emotions, anger, suspicion, blame, gossip
...
dotnet run --project AngryGuy.Tests                 # 39 tests over the AI and social systems
...
Every one of them has an inner life beyond anger: they get bored, frightened,
embarrassed, smug, and amused at each other's misfortune. Gordon dislikes Terry,
so if Terry goes over on a wet floor in front of him, Gordon **laughs** - which
humiliates Terry, damages their relationship, and makes Terry more likely to
blame Gordon for the next thing that goes wrong.

They also notice patterns. Every mishap nudges an NPC's **wariness**, and once
enough has gone wrong they stop believing in coincidence, say so out loud, and
start actively hunting for a culprit. Break everything at once and you end up
working in a room full of people looking for you - the difficulty curve is the
cast working it out, not a number going up.
...
- **Disguises.** The kitchen and wash-up are staff-only: loitering there in your
  own clothes is quietly incriminating all on its own. Steal chef's whites, a
  waiter's apron or cleaner's overalls from the staff lockers and that pressure
  disappears - and carrying the chef's own pan across his own kitchen stops
  looking like theft. It hides your role, not your face: Eva knows the staff, and
  given long enough she will work it out.
- **The light switch.** Kills everyone's sight range at once. Any tidy-minded NPC
  will put them back on.
- **The fire alarm.** Empties the building for half a minute. Also the single
  most incriminating thing in the level if anyone sees you pull it.
...
```bash
python3 tools/assets/generate_audio.py      # 30 clips: effects, 4 character voices, music
python3 tools/assets/generate_textures.py   # 5 seamless textures
```

Each of the four staff has their own synthesised voice — a pitch, a rasp and a
cadence — so you can tell who just shouted in the next room without looking.
